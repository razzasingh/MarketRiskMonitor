...
- **Liquidity-at-Risk (LaR)** — peak cumulative loss over each instrument's liquidation horizon under stress scenarios, used as a margin requirement proxy
- **Capital proxy (FRTB-style)** — model capital estimated as 1.5× max(ES, Stressed ES), plus quantified RNIM add-ons (liquidity, concentration, JTD, correlation)
- **Scenario stress testing** — explicit return-space shocks across rate, credit, FX, and macro scenarios, with desk-, factor-, and instrument-level attribution and concentration metrics (HHI, top-share) per scenario
- **Reverse stress testing** — computes the minimum shock multiplier required to breach the capital loss threshold for each scenario
